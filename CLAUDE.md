...
- **Dedicated browser instances with session reuse** — `launchRetailerBrowser(retailerKey)` launches (or reuses cached) a Chromium instance per retailer with its own proxy config, viewport, UA, and stored browser state. Browser+context cached in `retailerBrowserCache` within a poll — multiple stores for the same retailer (e.g., 5 Total Wine stores) share one browser instance with accumulated cookies. Pages are closed after each use; browsers closed at end of poll via `closeRetailerBrowsers()`.
