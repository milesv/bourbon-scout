...
- **`.github/workflows/bourbon-scout.yml`** — GitHub Actions cron job. Runs every 15 min (offset to `:03/:18/:33/:48`). Caches Playwright Chromium and state files between runs. Uses `RUN_ONCE=true`.
...
- **Discord embeds per-store** — In-stock finds trigger `@everyone` urgent alerts immediately. A quiet summary embed posts at the end of every poll. One embed per webhook call to stay under Discord's 6000 char payload limit.
