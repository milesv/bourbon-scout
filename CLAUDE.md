...
- **Fetch-first with browser fallback** — Walmart and Sam's Club try plain HTTP fetch first (via residential proxy) before spinning up a browser page. Walmart parses `__NEXT_DATA__`, Sam's Club parses `__NEXT_DATA__` per-product. Falls back to Playwright if blocked. Costco and Total Wine skip fetch-first (Akamai/PerimeterX consistently block node-fetch due to TLS fingerprint) and go straight to dedicated browser instances.
...
|----------|-------------|-------------|----------|-------|
| Costco | Playwright (dedicated browser) | MUI `data-testid` DOM attrs | Akamai Bot Manager | Scrape-once (no store filter in search). Skips fetch-first (Akamai blocks node-fetch TLS). Dedicated browser instance via `launchRetailerBrowser("costco")` with per-retailer IP. Next.js App Router — no `__NEXT_DATA__`. |
| Total Wine | Playwright (dedicated browser) | `window.INITIAL_STATE` JSON | PerimeterX (HUMAN) | Skips fetch-first (PerimeterX blocks node-fetch TLS). Dedicated browser instance via `launchRetailerBrowser("totalwine")` with per-retailer IP. Per-store via `storeId` URL param. Homepage pre-warm before searching. |
...
| `test/proxy.test.js` | 32 | Proxy agent routing, SOCKS5/HTTP protocol auto-detection, fetch-first paths (Walmart, Sam's Club), Costco/TotalWine dedicated browser wrappers, Costco cookie pre-warm, Costco blocked retry recovery |
