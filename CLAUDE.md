...
- **Human-like inter-query pacing** — Browser scrapers wait 2-4s (randomized) between search queries instead of the old 750ms. Fetch scrapers wait 1-2s. Real humans take 5-15s between searches, so this interval is still faster but much less suspicious than sub-second bursts.
