...
- **Dedicated browser instances** — `launchRetailerBrowser(retailerKey)` launches a separate Chromium instance per retailer with its own proxy config, viewport, UA, and stored browser state. Each browser is closed after use. Used by Costco, Total Wine, Walmart (fallback), Walgreens, and Sam's Club (fallback).
