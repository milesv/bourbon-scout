...
656 tests across 5 files using [Vitest](https://vitest.dev/):
...
| File | Tests | Focus |
|------|-------|-------|
| `test/scraper.test.js` | 530 | Bottle matching, per-retailer filtering, EXCLUDE_TERMS, miniature filter, all 7 scrapers, Discord embeds, poll orchestration, error isolation, health tracking, scan metrics/trends, retry mechanisms, bot detection, state management, browser mutex, proxy rotation, challenge solver, schedule-aware polling, known URL tracking, priority-based query rotation, Chrome header order/GREASE brand, env validation, Discord 5xx/network retry |
