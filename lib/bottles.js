...
//   obsess:        every scan re-alerts; always urgent @here; ignores re-alert N
//   track:         standard cadence (REALERT_EVERY_N_SCANS); standard urgent alert
//   lead_suppress: yellow "🔍 LEAD" alerts are silenced for these bottles, but
//                  confirmed @here pings, ♻️ RESTOCK alerts, and 🔥 cross-source
//                  confirmations still fire normally. State.json keeps tracking
//                  so the data is queryable via scripts/find.js. Use for common
//                  bottles where weak-signal Kroger lead alerts are more noise
//                  than value (e.g., a single-facing planogram slot for E.H.
//                  Taylor Small Batch — common enough that 17 yellow embeds
//                  per scan adds up; we want notification only when stock is
//                  actually confirmed).
//   ignore:        silently records state, NO Discord alerts at all (useful for
//                  "found but not interested" cases like clearance JD 14yr at
//                  MSRP+200%)
//
// Bottles not listed here default to "track". The classification is checked
// in buildStoreEmbeds (lead_suppress filtering) and recordResult (ignore
// filtering); both run AFTER updateStoreState so state always reflects truth.
export const BOTTLE_INTEREST_TIERS = {
  ...
  track: [], // populated implicitly — anything not in obsess/ignore/lead_suppress is "track"
  lead_suppress: [
...
export const LEAD_SUPPRESSED_BOTTLES = new Set(BOTTLE_INTEREST_TIERS.lead_suppress);
...
export function bottleInterestTier(bottleName) {
  ...
  if (LEAD_SUPPRESSED_BOTTLES.has(bottleName)) return "lead_suppress";
  return "track";
}

// Returns true if a "lead" alert for this bottle should be silenced.
// Confirmed/restock/cross-source alerts still fire normally.
export function isLeadSuppressed(bottleName) {
  return LEAD_SUPPRESSED_BOTTLES.has(bottleName);
}
