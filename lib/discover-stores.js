...
import { FALLBACK_STORES, EXTRA_STORES } from "./fallback-stores.js";

const CACHE_FILE = new URL("../stores.json", import.meta.url);
const CACHE_TTL_DAYS = 7;
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

// Format a 10-digit phone string into "(XXX) XXX-XXXX". Tolerates any input
// (digits-only, formatted, undefined, non-string) — returns undefined if it
// can't extract exactly 10 US-style digits. Used by retailer locator
// implementations (currently Kroger) that capture raw phone from upstream APIs.
// Output matches the format used in lib/fallback-stores.js so embed rendering
// is consistent regardless of which path provided the store.
export function formatPhoneFromDigits(raw) {
  if (!raw) return undefined;
  const digits = String(raw).replace(/\D/g, "");
  if (digits.length === 11 && digits.startsWith("1")) {
    // Strip leading "1" country code if present (some APIs include it)
  ...
  for (const [key, stores] of Object.entries(retailers)) {
    ...
    retailers[key] = stores.filter((s) => {
      ...
      name: s.name
        ...
        .replace(/^(Safeway|Albertsons|Costco|Walmart|Total Wine|Walgreens)\s+\1\s+/i, "$1 ")
        .trim(),
    }));
  }
  return retailers;
...
async function locateKroger(zip, coords, radiusMiles, maxStores, clientId, clientSecret) {
  ...
  try {
    ...
    for (const loc of data.data || []) {
      ...
      if (dist && dist > radiusMiles) continue;
      // Capture phone from Kroger Locations API (returns 10-digit string with no
      // formatting, e.g., "4806001234"). Format to "(480) 600-1234" so it matches
      // the rendering in the rest of the codebase. The Discord embed pipeline
      // (`formatStoreInfo` → `phoneLine` / `_alertContent`) reads `store.phone`
      // directly, so any non-empty phone field surfaces in confirmed/lead/restock
      // alerts as a tap-to-call link + lock-screen-visible push notification.
      const formattedPhone = formatPhoneFromDigits(loc.phone);
      stores.push({
...
async function locateAlbertsons(page, zip, coords, radiusMiles, maxStores) {
  const stores = [];
  try {
    // Albertsons uses the same Yext-powered locator as Safeway (same parent company)
    await page.goto(`https://local.albertsons.com/search.html?q=${zip}`, {
      waitUntil: "networkidle",
      timeout: 45000,
    });
    await page.waitForTimeout(5000);
    ...
    const results = await page.evaluate(
      ...
      () => {
        const items = [];
        const cards = document.querySelectorAll(
          '[data-ya-scope="result"], .LocationCard, a[href*="/albertsons/"]'
        );
        for (const card of cards) {
          const nameEl = card.querySelector('[class*="name"], [class*="title"], h2, h3, .location-name');
          const addrEl = card.querySelector('[class*="address"], address, .location-address');
          const distEl = card.querySelector('[class*="distance"], [class*="miles"]');
          const href = card.tagName === "A" ? card.href : card.querySelector("a")?.href || "";
          const idMatch = href.match(/\/albertsons\/[^/]+\/[^/]+\/(\d+)/);
          const name = nameEl?.textContent?.trim() || "";
          items.push({
            name,
            address: addrEl?.textContent?.trim() || "",
    ...
    for (const r of results) {
      ...
      const cleanName = r.name.replace(/[\d.]+\s*mi\b.*$/i, "").replace(/\u00a0/g, " ").trim();
      if (!cleanName) continue;
      const sid = r.id || `ab-${stores.length}`;
      if (r.id && seen.has(r.id)) continue;
      if (r.id) seen.add(r.id);
      stores.push({
        storeId: sid,
        name: cleanName.startsWith("Albertsons") ? cleanName : `Albertsons ${cleanName}`,
        address: r.address,
        distanceMiles: dist,
      });
    }
  } catch (err) {
    console.error(`[discover] Albertsons locator failed: ${err.message}`);
  }
  return stores;
}
...
export async function discoverStores({ zipCode, radiusMiles = 15, maxStores = 5, krogerClientId, krogerClientSecret, krogerMaxStores, krogerRadiusMiles }) {
  // Check cache first
  ...
  // Kroger/Fry's is pure REST API (no browser, no anti-bot) so it can handle many more stores
  // than browser-based retailers. KROGER_MAX_STORES and KROGER_RADIUS_MILES override defaults.
  const krogerPromise = locateKroger(zipCode, coords, krogerRadiusMiles || radiusMiles, krogerMaxStores || maxStores, krogerClientId, krogerClientSecret)
  ...
  try {
    ...
    const browserLocators = [
      ...
      { key: "albertsons", fn: locateAlbertsons },
  ...
  // ExtraMile is a single Chevron store with hardcoded merchant ID — no discovery
  // needed, just use the fallback. Always emit at least one entry so the daemon
  // knows to scan it.
  retailers.extramile = FALLBACK_STORES.extramile || [];
  console.log(`[discover] Using ${retailers.extramile.length} extramile store(s) (single-store retailer)`);

  // Merge EXTRA_STORES — user-specified stores outside the discovery radius
  for (const [key, extras] of Object.entries(EXTRA_STORES)) {
    if (!retailers[key]) retailers[key] = [];
    const existingIds = new Set(retailers[key].map((s) => s.storeId));
    for (const store of extras) {
      if (!existingIds.has(store.storeId)) {
        retailers[key].push(store);
        console.log(`[discover] Added extra store: ${store.name} (${key} #${store.storeId})`);
      }
    }
  }
