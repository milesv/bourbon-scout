...
//
// `distanceMiles` values are APPROXIMATE — geographic estimates from 85283
// (Tempe, AZ) accurate to within ~1-2 miles. Used purely for sorting and the
// "X mi" rendering in Discord embeds; not safety-critical. The daemon recomputes
// exact haversine distances during fresh store discovery from each retailer's
// own locator API, but those lookups have been failing consistently in 2026
// (Costco HTTP/2 errors, Total Wine PerimeterX, Walmart Akamai) — so these
// approximations are what actually surfaces in alerts most of the time.
...
// OR within radius but not surfaced by the retailer's store finder. Always
// merged after discovery (unlike FALLBACK_STORES which only apply when
// discovery returns 0). Deduped by storeId to prevent duplicates.
//
// Mesa #1028 (1444 S Sossaman Rd) is technically within the 15mi search
// radius but Costco's store finder doesn't return it for ZIP 85283 — adding
// it here guarantees east-Mesa coverage between SE Gilbert (#644, 10.5mi)
// and Paradise Valley (#1058, 19mi).
